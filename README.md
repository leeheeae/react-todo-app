...
-   useCallback을 사용할 때 두 번째 파라미터로 넣는 배열에 number을 넣지 않아도 된다.

**useReducer 사용하기**

-   useReducer를 사용할 때 원래 두 번째 파라미터에 초기 상태를 넣어주어야 하지만 지금은 그 대신 두 번째 파라미터에 undifined를 넣고, 세 번째 파라미터에 초기 상태를 만들어주는 함수인 createBulkTodos를 넣어 컴포넌트가 맨 처음 렌더링 될 때만 함수를 호출시킴
-   기존 코드를 많이 고쳐야하는 단점이 있지만, 상태를 업데이트 하는 로직을 모아서 컴포넌트 바깥에 둘 수 있다는 장점이 있음

**react-virtualized를 사용한 렌더링 최적화**

> 스크롤되기 전에 보이지 않는 컴포넌트는 렌더링하지 않고 크기만 차지하게끔 할 수 있음
> 스크롤되면 해당 스크롤 위치에서 보여 주어야 할 컴포넌트를 자연스럽게 렌더링

-   `npm i react-virtualized` 설치
